from aeternity import exceptions, __compiler_compatibility__, __compiler_compatibility_prelima__
from aeternity import utils, defaults, hashing, openapi, identifiers
from aeternity.exceptions import UnsupportedNodeVersion
...
class CompilerFactory:
    """
    Helper to get the right complier version based on the node protocol that is targeted
    """
    @staticmethod
    def get_compiler(cls, compiler_url='http://localhost:3080', compiler_options=None):
        """
        Get the right compiler client based on the protocol used
        """
        try:
            return CompilerClient(compiler_url=compiler_url)
        except UnsupportedNodeVersion:
            return CompilerClientFate(compiler_url=compiler_url)
...
class CompilerClientFate(object):
    """
    ...
    """
    def __init__(self, compiler_url='http://localhost:3080', compiler_options={"backend": identifiers.COMPILER_OPTIONS_BACKEND_FATE}):
        ...
        self.compiler_options = {"backend": identifiers.COMPILER_OPTIONS_BACKEND_FATE} if compiler_options is None else compiler_options
...
class CompilerClient(object):
    """
    The compiler client Fate version is the client for the aesophia_http compiler v3.2.x series,
    that is compatible up to FORTUNA protocol (v3)
    """

    def __init__(self, compiler_url='http://localhost:3080', **kwargs):
        self.compiler_url = compiler_url
        self.compiler_cli = openapi.OpenAPICli(compiler_url, compatibility_version_range=__compiler_compatibility_prelima__)
    ...
    def encode_calldata(self, source_code, function_name, arguments=[], **kwargs):
        if not isinstance(arguments, list):
            arguments = [arguments]
        if arguments is None:
            arguments = []
    ...
    def decode_data(self, sophia_type, encoded_data, **kwargs):
        body = {
            "data": encoded_data,
            "sophia-type": sophia_type
        }
    ...
    def decode_calldata_with_bytecode(self, bytecode, encoded_calldata, **kwargs):
        body = {
            "calldata": encoded_calldata,
            "bytecode": bytecode
        }
    ...
    def decode_calldata_with_sourcecode(self, sourcecode, function, encoded_calldata, **kwargs):
        body = {
            "source": sourcecode,
            "function": function,
            "calldata": encoded_calldata
